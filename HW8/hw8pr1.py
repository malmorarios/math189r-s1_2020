...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			datapoint = X[i,:]
			distanceToClusters = np.linalg.norm(datapoint - clusters, axis=1)
			clusterLabel = np.argsort(distanceToClusters).item(0)
			label[i] = clusterLabel

		for i in range(k):
			indecesOfDatapoints_ithCluster = np.where(label == i)[0]
			if len(indecesOfDatapoints_ithCluster) > 0: # -- took hint from solution file. Why would this ever be 0 though?
				clusters[i, :] = X[indecesOfDatapoints_ithCluster].mean(axis=0)
...
def k_means_cost(X, clusters, label):
	...
	"*** YOUR CODE HERE ***"
	labeledClusters = clusters[label.flatten()]
	cost = math.sqrt((np.linalg.norm(X - labeledClusters, axis=1) ** 2).sum())
...
if __name__ == '__main__':
	...
	cost_k_list = []
	for k in range(1, 21): 							# where, I'm assuming, the 20 upper bound was chosen randomly
		# TODO: Get the clusters, labels, and cost list for different k values
		"*** YOUR CODE HERE ***"
		clusters, labels, cost_list = k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10)
	...
	"*** YOUR CODE HERE ***"
	cost_vs_k, = plt.plot(range(1, 21), cost_k_list, 'g^')
	opt_cost, = plt.plot(opt_k, min(cost_k_list), 'rD') # looked at solution file for that last parameter (g^ and rD)
	...
	"*** YOUR CODE HERE ***"
	clusters, labels, cost_list = k_means(X, opt_k)
	labeledClusters = clusters[labels.flatten()]
	data_plot, = plt.plot(X[:, 0], X[:, 1], 'bo') # got help here too
	center_plot, = plt.plot(labeledClusters[:, 0], labeledClusters[:, 1], 'rD')
