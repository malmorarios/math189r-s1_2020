...
if __name__ == '__main__':
	...
	#flattenedimage = img.copy().flatten()
	#shuffle_img = np.random.shuffle(shuffle_img)
	...
	"*** YOUR CODE HERE ***" # had to use help from solution guide
	orig_S_plot, = plt.plot(S[:k], 'g')
	shuf_S_plot, = plt.plot(S_s[:k], 'y')
	...
	for index in range(len(rank_list)):
		...
		"*** YOUR CODE HERE ***" # got help here too
		image = U[:, :k] @ np.diag(S)[:k, :k] @ V[:k, :]
