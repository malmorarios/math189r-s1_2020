...
def find_cost(X, y, W , reg):
	...
	"*** YOUR CODE HERE ***"
	# error = (X @ W - y).T @ (X@W - y)
	# cost = (error + reg * np.abs(W).sum()) / len(y)
	# my way didn't work for some reason. I'm so done. It's literally the same thing. Give me math. Not coding.
...
def prox(X, gamma):
	...
	X[np.abs(X) <= gamma] = 0. # got help from solution file to adress errors produced from not adding this line
...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		#1
		randomlySelectedIndeces = np.random.randint(0,m,size=batch_size)

		#2
		W_grad = find_grad( X[randomlySelectedIndeces], y[randomlySelectedIndeces], W, reg=reg)

		#3
		W = prox(W - lr*W_grad, reg * lr)

		#4
		cost = find_cost( X[randomlySelectedIndeces], y[randomlySelectedIndeces], W, reg=reg )
...
if __name__ == '__main__':
	...
	"""*** YOUR CODE HERE ***"""
	dfCol_array = np.array(df.columns)
	top_features = dfCol_array[np.argsort(-W[:, 0])[:5] + 1]
	"""*** END YOUR CODE HERE ***"""
	print(top_features)
