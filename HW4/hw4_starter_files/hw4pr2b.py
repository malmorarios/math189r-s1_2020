...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	exp = np.exp(X@W)
	mu_ic = exp / exp.sum(axis=1).reshape(-1, 1)
	insideTheSum = y * np.log(mu_ic)
	NLL = -insideTheSum.sum(axis=1).sum() + reg * np.diag(W.T @ W).sum()
...
def predict(X, W):
	...
	exp = np.exp(X@W)
	mu_ic = exp / exp.sum(axis=1).reshape(-1, 1)
	y_pred = np.argmax(mu_ic, axis=1).reshape(-1, 1)
	return y_pred
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	"*** YOUR CODE HERE ***"
	optimalLambda = np.argmax(accu_list)
	reg_opt = lambda_list[optimalLambda]
