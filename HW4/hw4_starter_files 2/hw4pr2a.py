...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X@W)
	insideTheSum = np.multiply( y, np.log(mu) ) + np.multiply( (1-y), np.log(1-mu) ) #ignored 1. way of writing numbers
	nll = (-sum(insideTheSum) + reg/2 * np.linalg.norm(W)**2).item(0)
...
def newton_step(X, y, W, reg=0.0):
	...
	H = X.T @ diag @ X + reg * np.identity(X.shape[1])
...
def get_description(X, y, W):
	...
	"*** YOUR CODE HERE ***"
	predictedLabels = predict(X,W)

	accuracyCount = 0

	precisionCount = 0
	calledNumOfPositives = 0

	recallCount = 0
	trueNumOfPositives = 0	

	for i in range(X.shape[0]):
		actual, prediction = y.item(i), predictedLabels.item(i)
		if actual == prediction:
			accuracyCount +=1
		if prediction == 1:
			calledNumOfPositives += 1
			if actual == 1:
				precisionCount += 1
		if actual == 1:
			trueNumOfPositives += 1
			if prediction == 1:
				recallCount += 1
	
	accuracy = accuracyCount / X.shape[0]
	precision = precisionCount / calledNumOfPositives
	recall = recallCount / trueNumOfPositives
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0, 10, 0.1, 40, 0.00001, 5, 100, 25, 0.05, 15] 
	reg_list.sort()
	for index in range(len(reg_list)):
		reg = reg_list[index]
		W_opt, obj = newton_method(X_train, y_train, reg = reg, \
			print_freq = 100)
	...
	"*** YOUR CODE HERE ***"
	lambda_index = np.argmax(a_list)
	reg_opt = reg_list[lambda_index]
